...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, relative_geometry_weights=None):
        '''
        Computes
        ...
        :return:
        '''
        b_s, nq, d_model = queries.shape[:3]
        nk = keys.shape[1]
...
class VCGA(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1):
        ...
        :param h: Number of heads
        '''
        super(VCGA, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_o = nn.Linear(h * d_v, d_model)

        self.Adaptive_weight = nn.Linear(d_k * 2, 1)
    ...
    def init_weights(self):
        ...
        nn.init.xavier_uniform_(self.fc_v.weight)
        nn.init.xavier_uniform_(self.fc_o.weight)
        nn.init.xavier_uniform_(self.Adaptive_weight.weight)

        nn.init.constant_(self.fc_q.bias, 0)
        nn.init.constant_(self.fc_k.bias, 0)
        nn.init.constant_(self.fc_v.bias, 0)
        nn.init.constant_(self.fc_o.bias, 0)
        nn.init.constant_(self.Adaptive_weight.bias, 0)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None,
        ...
        if attention_mask is not None:
            att = att.masked_fill(attention_mask, -np.inf)
        # relative_geometry_weights: (b_s, 49, 49, 64)
        # (b_s, nq, nk, 64)
        rpe = relative_geometry_weights
        # rpe_t is [batch, num queries, depth, num kvs]
        rpe_t = rpe.transpose(-2, -1)
        #   [batch, num queries, heads, depth]
        # @ [batch, num queries, depth, num kvs]
        # = [batch, num queries, heads, num kvs]
        # q is (b_s, h, nq, d_k), q_t is [batch, num queries, heads, depth]
        q_t = q.permute(0, 2, 1, 3)
        q_rpe_t_matmul = torch.matmul(q_t, rpe_t)
        # [batch, heads, num queries, num kvs]
        q_rpe_t_matmul = q_rpe_t_matmul.permute(0, 2, 1, 3)
        # att is (b_s, h, nq, nk)
        att = att + q_rpe_t_matmul
        att = att / np.sqrt(self.d_k)
        att = torch.softmax(att, -1)
        att = self.dropout(att)

        # out_v is [batch, heads, num queries, depth]
        # out_v_t is [batch, num queries, heads, depth]
        # v is [b_s, h, nk, d_v]
        out_v = torch.matmul(att, v)
        # att_t is [b_s, nq, h, nk]
        att_t = att.permute(0, 2, 1, 3)
        # [b_s, nq, h, nk] @ [b_s, nq, nk, 64]
        # = [batch, num queries, heads, depth] -> [batch, heads, num queries, depth]
        out_rpe = torch.matmul(att_t, rpe).permute(0, 2, 1, 3)

        # [batch, heads, num queries, depth * 2]
        f = torch.cat((out_v, out_rpe), dim=-1)
        rpe_weight = torch.sigmoid(self.Adaptive_weight(f))
        out = (out_v + rpe_weight * out_rpe).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)
