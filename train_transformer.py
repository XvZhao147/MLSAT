...
from models.transformer.attention import VCGA
...
def train(rank, worldSize, args):
    ...
    encoder = TransformerEncoder(3, 0, num_semantics=args.num_semantics, attention_module=VCGA, attention_module_kwargs={'m': args.m})
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if val_cider >= best_cider:
            ...
            patience = 0
            best = True
            print('\n val_cider >= best_cider, patience = ', patience)
        else:
            patience += 1
            print('\n val_cider < best_cider,patience = ', patience)
        ...
        if rank == 0:
            ...
            if best:
                copyfile('/saved_transformer_models/%s_last.pth' % args.exp_name,
                         '/saved_transformer_models/%s_best.pth' % args.exp_name)
            if best_test:
                copyfile('/saved_transformer_models/%s_last.pth' % args.exp_name,
                         '/saved_transformer_models/%s_best_test.pth' % args.exp_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='MLSA Transformer')
